...
import { Siren, AlertTriangle, MapPin, ArrowRightCircle, ChevronDown, ChevronUp, XCircle } from 'lucide-react';
import { CATEGORY_COLORS } from '../../constants/config';

export default function LogTablesSection({ logData, accidentLogData, specialLaneLogData, onManualClose, manuallyClosedLanes = [] }) {
  ...
  // Data is now pre-processed by the useFilteredData hook
  const enhancedLanes = Array.isArray(specialLaneLogData) ? specialLaneLogData : [];
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
      ...
      <div className="bg-slate-800 rounded-lg border border-slate-700 shadow-md flex flex-col h-[400px] overflow-hidden hover-lift-glow">
        <div className="px-4 py-3 bg-slate-900/80 border-b border-slate-700 flex justify-between items-center"><h3 className="text-white text-sm font-bold flex items-center gap-2"><Siren size={16} className="text-yellow-500" /> รายการเหตุการณ์ (Log)</h3><span className="text-xs text-slate-500 bg-slate-800 px-2 py-1 rounded border border-slate-600">{logData.length > 200 ? `แสดงล่าสุด 200/${logData.length} รายการ` : `แสดง ${logData.length} รายการ`}</span></div>
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          <table className="w-full text-xs text-left text-slate-300">
            ...
            <tbody className="divide-y divide-slate-700/50">
              {logData.length > 0 ? logData.slice(0, 200).map((item, idx) => (
                      ...
                      <tr key={idx} className="hover:bg-green-900/20 transition-colors bg-green-950/20">
                        ...
                        <td className="px-3 py-3 align-top">
                          ...
                          <div className="mt-1.5 flex flex-wrap gap-1">
                            ...
                            <button 
                              onClick={() => onManualClose && onManualClose(lane.id)}
                              className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] bg-red-600 hover:bg-red-700 text-white shadow-sm border border-red-500 transition-colors"
                            >
                              <XCircle size={10} /> ปิดงานนี้
                            </button>
                          </div>
                        </td>
                      </tr>
                          ...
                          <td className="px-3 py-3 align-top">
                            ...
                            <div className="mt-1.5 flex flex-wrap gap-1">
                              <span className="inline-block px-1.5 py-0.5 rounded text-[10px] font-bold bg-slate-700 text-slate-300">
                                🔴 ปิดแล้ว {lane.closeInfo ? `${lane.closeInfo.time} น.` : '(โดยผู้ใช้)'}
                              </span>
                              {lane.durationText && (
      ...
      <div className="bg-slate-800 rounded-lg border border-slate-700 shadow-md flex flex-col h-[400px] overflow-hidden hover-lift-glow">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar bg-slate-900/30">
          <table className="w-full text-xs text-left text-slate-300">
            ...
            <tbody className="divide-y divide-slate-700/50">
              {accidentLogData.length > 0 ? accidentLogData.slice(0, 200).map((item, idx) => (
