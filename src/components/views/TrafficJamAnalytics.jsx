...
export default function TrafficJamAnalytics({ filteredData }) {
    
    // Process Data
    const congestionStats = useMemo(() => {
        const trafficData = (filteredData || []).filter(d => 
            d.category && (
                d.category === 'จราจรติดขัด' || 
                d.category === 'จราจรหนาแน่น' ||
                d.category.includes('หนาแน่น') || 
                d.category.includes('รถติด') || 
                (d.detail && (d.detail.includes('รถมาก') || d.detail.includes('หนาแน่น')))
            )
        );
