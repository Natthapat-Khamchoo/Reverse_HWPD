...
// 🗓️ ตัวแปลงวันที่ให้เป็นมาตรฐานเดียวกัน (YYYY-MM-DD) เสมอ
const standardizeDate = (dateStr) => {
  if (!dateStr) return "";
  const cleanDate = dateStr.trim();
  // กรณี 2025/12/26 -> 2025-12-26
  if (/^\d{4}[\/\-]\d{1,2}[\/\-]\d{1,2}/.test(cleanDate)) return cleanDate.replace(/\//g, '-');
  // กรณี 26/12/2025 -> 2025-12-26
...
// 👮‍♂️ ตัวแกะหน่วยงานฉบับอัปเกรด (ยืดหยุ่นสูง)
// รองรับ: "กก.1", "กก. 1", "กก 1", "กก.01" -> แปลงเป็น "1" หมด
const extractUnitFromText = (text) => {
    if (!text) return { div: '', st: '' };
    
    // Regex จับ กก. (มีจุดหรือไม่มี, มีวรรคกี่ตัวก็ได้)
    const divMatch = text.match(/กก\.?\s*(\d+)/);
    // Regex จับ ส.ทล.
    const stMatch = text.match(/ส\.?ทล\.?\s*(\d+)/);
    
    return {
        // ใช้ parseInt เพื่อแปลง "01" เป็น "1" ให้ตรงกับ value ใน Dropdown
        div: divMatch ? parseInt(divMatch[1], 10).toString() : '',
        st: stMatch ? parseInt(stMatch[1], 10).toString() : ''
    };
};
...
export default function App() {
  ...
  const fetchData = useCallback(async (isBackground = false) => {
    ...
    try {
      ...
      const combinedData = [...dataTraffic, ...dataEnforce, ...dataSafety].map(item => {
          ...
          // 🔥 Fix: บังคับแกะหน่วยงานใหม่ (Force Re-parse)
          // ถ้า item.div เดิมไม่มี หรืออยากชัวร์ ให้ใช้ฟังก์ชันใหม่แกะจาก Text
          let newDiv = item.div;
          let newSt = item.st;
          
          // ถ้ามีข้อความหน่วยงาน ให้ลองแกะใหม่เสมอ (เผื่อ dataProcessor พลาดเคส กก.1)
          if (item.unit) {
             const extracted = extractUnitFromText(item.unit);
             // ถ้าแกะได้ค่า (ไม่ว่างเปล่า) ให้ใช้ค่าที่แกะได้ทันที
  ...
  const stats = useMemo(() => {
    ...
    // Bar Chart: เปรียบเทียบทุกหน่วยเสมอ
  ...
  // ... (ส่วน Report และ JSX เหมือนเดิม)
