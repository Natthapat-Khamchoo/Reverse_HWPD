...
export default function MapAndChartSection({ mapData, stats, handleChartClick, LONGDO_API_KEY, currentView }) {
   return (
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 mb-4 h-auto lg:h-[450px]">
         <div className="lg:col-span-8 bg-slate-800 rounded-lg border border-slate-700 relative overflow-hidden shadow-md flex flex-col h-[350px] lg:h-full">
            <div className="absolute top-2 left-2 z-[400] bg-slate-900/90 px-3 py-1.5 rounded border border-slate-600 text-[10px] text-white font-bold flex items-center gap-2 shadow-sm">
               <MapIcon size={12} className="text-yellow-400" /> ภาพรวม {currentView === 'accident' && '(โหลดเสี่ยงอุบัติเหตุ)'}
            </div>
            <div className="flex-1 w-full h-full"><LongdoMapViewer data={mapData} apiKey={LONGDO_API_KEY} currentView={currentView} /></div>
         </div>
         <div className="lg:col-span-4 bg-slate-800 p-4 rounded-lg border border-slate-700 shadow-md flex flex-col h-[300px] lg:h-full hover-lift-glow">
