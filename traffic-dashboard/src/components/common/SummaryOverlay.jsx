...
import { ClipboardCopy, ArrowRightCircle, Terminal } from 'lucide-react';

export default function SummaryOverlay({ reportText, onCopy, onEnter, copySuccess }) {
    // Removed Typewriter effect as per user request to show text all at once.
    // Using CSS animation 'slide-in-from-bottom-5 fade-in duration-700' for the text block.
    ...
    return (
        <div className="fixed inset-0 z-[9000] bg-slate-950/90 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in-delayed">
            <div className="bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl max-w-3xl w-full max-h-[90vh] flex flex-col overflow-hidden animate-warp-in ring-1 ring-slate-700/50">
                ...
                {/* Header */}
                <div className="p-6 border-b border-slate-800 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 flex items-center justify-between relative overflow-hidden">
                    {/* Scanline Effect */}
                    ...
                    <span className="relative z-10 text-xs font-mono text-emerald-400 bg-emerald-950/30 px-3 py-1 rounded border border-emerald-500/20 flex items-center gap-2">
                ...
                <div className="p-6 overflow-y-auto custom-scrollbar flex-1 bg-slate-950/50 relative">
                    <div className="bg-black/40 p-6 rounded-lg border border-slate-800 text-slate-300 font-mono text-sm whitespace-pre-wrap leading-relaxed shadow-inner min-h-[300px] animate-in slide-in-from-bottom-10 fade-in duration-1000 delay-500 fill-mode-forwards">
                        {reportText || "กำลังสร้างรายงาน..."}
                    </div>
                </div>
